...
const Home: NextPage = () => {
    ...
    return <Site>
        <div className="grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 w-full  py-4">
            {candidates?.sort((a, b) => b.stock - a.stock).sort((a, b) => (a.terminated ? 1 : 0) - (b.terminated ? 1 : 0)).map(candidate => {
                const ownedStocks = user?.Stock.find(stock => stock.candidateName === candidate.name)!.amount;
                return <div
                    key={candidate.name}
                    className={`
                    relative h-44 overflow-hidden flex flex-col justify-between select-none
                    ${candidate.terminated ? 'pointer-events-none cursor-auto' : 'cursor-pointer  border-light border shadow-lg'}
                    ${!candidate.terminated && ((selected === candidate.name) ? 'bg-primary text-white' : 'bg-white text-dark')}`}
                    onClick={() => setSelected(candidate.name)}
                >
                    <div
                        className={`bg-contain bg-no-repeat bg-top h-full absolute top-8 lg:top-0 left-0 w-full transition  ${selected === candidate.name ? 'scale-125' : ''}`}
                        style={{backgroundImage: `url(${candidate.picture})`}}/>
                    <div
                        className={`font-display p-4 text-lg font-bold transition  ${selected === candidate.name ? 'text-xl' : ''}`}>
                        {candidate.name}
                    </div>
                    ...
                    <div className="flex justify-between items-end relative">
                        <div className={ownedStocks && !candidate.terminated
                            ? `m-4 px-2 text-xl font-bold ${selected === candidate.name ? 'text-white' : 'text-primary'}`
                            : 'opacity-0'
                        }>
                            {ownedStocks}
                        </div>
                        <div className="font-display p-4 text-5xl opacity-70">
                            {!candidate.terminated && <>
                                {price(calculateStockPrice(candidate.stock + 1), true)}
